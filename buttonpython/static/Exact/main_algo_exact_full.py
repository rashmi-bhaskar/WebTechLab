...
# inputF = "dataset_small.csv"
# r = csv.reader(open(inputF, encoding="utf8"))
# entry = list(r)
# entry.remove(entry[0])
# 
# features, hashvec = cleanup.csvClean(entry)
# features = preproc.extractFeatures(features)                # the final features is a list of entries where entry[i][0] is the original
                                                            # tweet and entry[i][1] is a string of features separated by '|'
...
for i in range(num_chunks-1):
    clusterdict, fvecs, freqdict = main_algo( features[chunksize*i : chunksize*(i+1)], tweetid, lastclusterid)
...
clusterdict, fvecs, freqdict = main_algo( features[chunksize*(num_chunks-1):], tweetid, lastclusterid)
for clusterid, tweetidlist in clusterdict.items():
    if( len(tweetidlist) > 1 ):
        clabel = clusterlabel.getClusterLabel(tweetidlist, fvecs, freqdict, top_n)
        towrite.append([ json.dumps([clusterid, clabel, len(tweetidlist)]) ])
...
outfile = open('clusterdata2_old.csv', 'w', newline='', encoding="utf8")
